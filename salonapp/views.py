...
def salon_display(request):
    ...
    return render(request, "salon.html", {"salon": salon})
...
def salon_views(request):
    email = request.session["email"]
    salon = Salons.objects.exclude(email=email)
    return render(request, "salon_display.html", {"salons": salon})
...
def salon_booking_delete(request, id):
    book = Booking.objects.get(id=id)
    book.delete()
    return redirect("/booking_display")
